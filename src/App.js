...
const lsCfg = (id) =>
  LS.find((s) => s.id === id) || { lbl: id, clr: '#4a5270', ico: '.' };
...
const fmtM = (n, c = '₺') =>
  n >= 1000 ? c + (n / 1000).toFixed(1) + 'K' : c + (n || 0);
...
function Logo({ size = 60 }) {
  return (
    <img
      src={LOGO_URL}
      alt="Hair International"
      style={{
        height: size,
        width: 'auto',
        maxWidth: '100%',
        objectFit: 'contain',
        filter: 'drop-shadow(0 2px 8px rgba(196,205,217,0.15))',
      }}
    />
  );
}
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '50%',
        flexShrink: 0,
        background: `linear-gradient(135deg,${clr || '#4f7cff'},${
          (clr || '#4f7cff') + '22'
        })`,
        border: `1px solid ${(clr || '#4f7cff') + '33'}`,
        display: 'flex',
        alignItems: 'center',
  ...
  return (
    <div>
      <div style={{ marginBottom: 22 }}>
        <div style={{ color: '#dde3ef', fontSize: 20, fontWeight: 900 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        {[
          ...
          >
            ...
            <div style={{ color: k.clr, fontSize: 24, fontWeight: 900 }}>
              {k.val}
            </div>
          </div>
        ))}
...
function Leads({ user, region, leads, setLeads }) {
  ...
  return (
    <div>
      ...
      >
        <div style={{ color: '#dde3ef', fontSize: 18, fontWeight: 900 }}>
          📋 Leadler
        </div>
        {can(user, 'edit_leads') && (
          <Btn onClick={() => setShowAdd(true)} sm>
            + Yeni Lead
          </Btn>
        )}
      </div>
      ...
      {filtered.map((l) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: '#dde3ef', fontWeight: 800, fontSize: 14 }}>
                {l.name}
              </div>
        ...
        <Modal title="Yeni Lead" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <Inp
              ph="Telefon"
              val={form.phone}
              set={(v) => setForm((f) => ({ ...f, phone: v }))}
            />
            <Sel
              val={form.status}
              set={(v) => setForm((f) => ({ ...f, status: v }))}
              opts={LS.map((s) => ({ v: s.id, l: s.ico + ' ' + s.lbl }))}
            />
            <Inp
              ph="Notlar"
              val={form.notes}
              set={(v) => setForm((f) => ({ ...f, notes: v }))}
              rows={3}
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn v="s" onClick={() => setShowAdd(false)}>
                Iptal
              </Btn>
      ...
      {sel && (
        <Modal title={sel.name} onClose={() => setSel(null)} wide>
          <div style={{ color: '#4a5270', fontSize: 12, marginBottom: 10 }}>
            📞 {sel.phone} · {lsCfg(sel.status).ico} {lsCfg(sel.status).lbl}
          </div>
          {sel.notes && (
            <div
              style={{
...
function Patients({ user, region, patients, setPatients, driveConnected }) {
  ...
  const save = async () => {
    if (!form.name) return;
    const tp = Number(form.totalPrice) || 0,
      dep = Number(form.deposit) || 0;
    ...
    const obj = {
      ...
      grafts: Number(form.grafts) || 0,
      totalPrice: tp,
      deposit: dep,
      remaining: Math.max(0, tp - dep),
      totalPaid: dep,
      status: 'planli',
      photos: JSON.stringify({}),
      source_type: form.sourceType,
      source_name: form.sourceType === 'acenta' ? form.sourceName : null,
      operator_name: operatorStr || null,
      region,
    };
    const saved = await insertHasta(obj);
    setPatients((ps) => [saved ?? { ...obj, id: Date.now() }, ...ps]);
    await logAction(
      user,
      region,
      'Hasta eklendi',
    ...
    if (region === 'suudi') {
      try {
        ...
        const suggestedFee =
          graftNum > 0 ? (graftNum < 3000 ? 400 : 500) : null;
        const notesParts = [];
        if (operatorStr) notesParts.push(`Operatör: ${operatorStr}`);
        if (suggestedFee)
          notesParts.push(
            `Önerilen ekip ücreti: $${suggestedFee} (${graftNum} greft)`
          );
  ...
  const handleUpload = async (e) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        alert(`${file.name} yuklenemedi`);
      }
    }
    const newPhotos = {
      ...currentPhotos,
      [activeTab]: allUrls,
      [`${activeTab}_videos`]: allVideos,
    };
  ...
  const handleDeletePhoto = async (url) => {
    ...
    const newPhotos = {
      ...photos,
      [activeTab]: (photos[activeTab] || []).filter((u) => u !== url),
    };
    const photosStr = JSON.stringify(newPhotos);
    await updateHasta(sel.id, { photos: photosStr });
    const updated = { ...sel, photos: photosStr };
  ...
  const shareWhatsApp = async () => {
    ...
    if (lastUpload?.driveLinks?.length) {
      msg += `\n🔗 *Drive:*\n`;
      lastUpload.driveLinks.forEach((l, i) => {
        msg += `${i + 1}. ${l}\n`;
      });
    }
  ...
  const toggleOperator = (name) => {
    setForm((f) => {
      ...
      return {
        ...f,
        operatorNames: has
          ? f.operatorNames.filter((n) => n !== name)
          : [...f.operatorNames, name],
      };
    });
  };
  ...
  return (
    <div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <div>
                  <div
                    style={{ color: '#dde3ef', fontWeight: 700, fontSize: 14 }}
                  >
                    {p.name}
                  </div>
                  <div style={{ color: '#4a5270', fontSize: 11 }}>
      ...
      {showAdd && (
        <Modal title="Yeni Hasta" onClose={() => setShowAdd(false)} wide>
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}
          >
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
                AD SOYAD *
              </div>
              <Inp
                ph="Ad"
            ...
            <div>
              ...
              <Inp
                type="date"
                ph=""
                val={form.surgeryDate}
                set={(v) => setForm((f) => ({ ...f, surgeryDate: v }))}
              />
            </div>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
              ...
              {form.grafts && (
                ...
                >
                  💡 Önerilen ekip ücreti: $
                  {Number(form.grafts) < 3000 ? '400' : '500'} (3000 altı: $400,
                  3000 ve üstü: $500)
                </div>
              )}
            </div>
            <div>
              ...
              <Sel
                val={form.technique}
                set={(v) => setForm((f) => ({ ...f, technique: v }))}
                opts={['FUE', 'DHI', 'Safir FUE', 'PRP']}
              />
            </div>
            {region === 'suudi' && (
              <div style={{ gridColumn: '1/-1' }}>
                <div
                  style={{ color: '#4a5270', fontSize: 10, marginBottom: 6 }}
                >
                  HASTAYI KİM YAPTI * (birden fazla seçilebilir)
                </div>
      ...
      {sel && (
        <Modal title={sel.name} onClose={() => setSel(null)} wide>
          {can(user, 'edit_patients') && (
            <button
              onClick={() => handleDeletePatient(sel)}
              style={{
                marginBottom: 14,
                padding: '10px 18px',
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ color: '#4a5270', fontSize: 11 }}>
              ⚗️ {sel.technique} · {sel.grafts} greft
            </div>
            {sel.operator_name && (
              <div
                style={{
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 20,
                    border: `1px solid ${
                      activeTab === c.id ? '#4f7cff' : '#1c2035'
                    }`,
                    background:
                      activeTab === c.id
                        ? 'rgba(79,124,255,0.15)'
                        : 'transparent',
                    color: activeTab === c.id ? '#4f7cff' : '#4a5270',
                    fontSize: 11,
                    fontWeight: 700,
                    cursor: 'pointer',
                  }}
...
function SuudiFinance({ user, region, patients, receivables, setReceivables }) {
  ...
  const savePayment = async () => {
    ...
    if (editItem) {
      ...
      if (updated)
        setPayments((ps) =>
          ps.map((p) => (p.id === editItem.id ? updated : p))
        );
      setEditItem(null);
    } else {
  ...
  const startEdit = (p) => {
    setForm({
      ...
      ali_haydar_fee: p.ali_haydar_fee || '',
      yusuf_fee: p.yusuf_fee || '',
      mete_fee: p.mete_fee || '',
      seyit_fee: p.seyit_fee || '',
      notes: p.notes || '',
    });
    setEditItem(p);
    setShowAdd(true);
  ...
  const totalRevenue =
    payments.reduce((s, p) => s + Number(p.total_price || 0), 0) +
    payments.reduce((s, p) => s + Number(p.seyit_fee || 0), 0);
  const totalAli = payments.reduce(
    (s, p) => s + Number(p.ali_haydar_fee || 0),
    0
  );
  ...
  const totalPending = pendingRec.reduce(
  ...
  const pendingAli = pendingRec
    .filter((r) => r.person === 'Ali Haydar')
    .reduce((s, r) => s + toUSD(r), 0);
  const pendingYusuf = pendingRec
    .filter((r) => r.person === 'Yusuf')
    .reduce((s, r) => s + toUSD(r), 0);
  const pendingMete = pendingRec
    .filter((r) => r.person === 'Mete')
    .reduce((s, r) => s + toUSD(r), 0);
  const pendingSeyit = pendingRec
    .filter((r) => r.person === 'Seyit')
    .reduce((s, r) => s + toUSD(r), 0);
  const pendingGenel = pendingRec
    .filter((r) => !r.person || r.person === 'Genel')
    .reduce((s, r) => s + toUSD(r), 0);
  ...
  const saveReceivable = async () => {
    ...
    let receiptUrl = null;
    if (recForm.receiptFile)
      receiptUrl = await uploadReceipt(recForm.receiptFile);
    const row = {
      region,
  ...
  const markPaid = async (rec) => {
    ...
    setReceivables((rs) =>
      rs.map((r) =>
        r.id === rec.id ? { ...r, paid: true, paid_date: dd(0) } : r
      )
    );
  };
    ...
    <div>
      ...
      >
        {[
          ...
          {
            lbl: 'Ali Haydar',
            val: `$${totalAli.toLocaleString()}`,
            clr: '#f97316',
          },
          {
            lbl: 'Yusuf',
            val: `$${totalYusuf.toLocaleString()}`,
            clr: '#f97316',
          },
          {
            lbl: 'Mete',
            val: `$${totalMete.toLocaleString()}`,
            clr: '#f97316',
          },
          {
            lbl: 'Seyit',
            val: `$${totalSeyit.toLocaleString()}`,
            clr: '#f97316',
          },
          {
            lbl: 'Net Kâr',
            val: `$${netProfit.toLocaleString()}`,
            clr: netProfit >= 0 ? '#22c55e' : '#f04040',
          },
        ].map((k) => (
          <div
            key={k.lbl}
          ...
          >
            ...
            <div style={{ color: k.clr, fontSize: 20, fontWeight: 900 }}>
              {k.val}
            </div>
          </div>
        ))}
              ...
              <tbody>
                {payments.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #1c2035' }}>
                    ...
                    <td style={{ color: '#4a5270', padding: '10px 10px' }}>
                      {p.technique}
                    </td>
                    <td
                      style={{
                    ...
                    <td style={{ color: '#f97316', padding: '10px 10px' }}>
                      ${Number(p.ali_haydar_fee || 0).toLocaleString()}
                    </td>
                    <td style={{ color: '#f97316', padding: '10px 10px' }}>
                      ${Number(p.yusuf_fee || 0).toLocaleString()}
                    </td>
                    <td style={{ color: '#f97316', padding: '10px 10px' }}>
                      ${Number(p.mete_fee || 0).toLocaleString()}
                    </td>
                    <td style={{ color: '#f97316', padding: '10px 10px' }}>
                      ${Number(p.seyit_fee || 0).toLocaleString()}
                    </td>
      ...
      >
        ...
        >
          💵 Net Ödenecek (Kişisel Alacaklar Düşülmüş)
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table
          ...
          >
            <thead>
              <tr style={{ borderBottom: '1px solid #1c2035' }}>
                {['Kişi', 'Hak Ediş', 'Bekleyen Alacak', 'Net Ödenecek'].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
            ...
            <tbody>
              {[
                ...
                },
                {
                  name: 'Seyit',
                  earned: totalSeyit,
                  pending: pendingSeyit,
                  net: netSeyit,
                },
              ].map((row) => (
                <tr
                  key={row.name}
                  style={{ borderBottom: '1px solid #1c2035' }}
                >
                  <td
                    style={{
                      color: '#dde3ef',
                  ...
                  <td style={{ color: '#22c55e', padding: '10px 10px' }}>
                    ${row.earned.toLocaleString()}
                  </td>
                  <td
                    style={{
                      color: row.pending > 0 ? '#f04040' : '#4a5270',
                      padding: '10px 10px',
                    }}
                  >
                    {row.pending > 0
        ...
        {pendingGenel > 0 && (
          ...
          >
            🏢 Genel/Şirket bekleyen: ${pendingGenel.toLocaleString()} (kişiye
            bağlı değil, ayrı takip edilir)
          </div>
        )}
      </div>
      ...
      >
        ...
        >
          <div>
            ...
            <div style={{ color: '#f97316', fontSize: 11, marginTop: 2 }}>
              Bekleyen: ₺{totalPending.toLocaleString()}
            </div>
          </div>
          <Btn sm onClick={() => setShowAddRec(true)}>
            + Alacak
          </Btn>
        </div>
        {pendingRec.length === 0 ? (
          ...
          pendingRec.map((r) => (
            ...
            >
              <div style={{ flex: 1, minWidth: 140 }}>
                <div
                  style={{ color: '#dde3ef', fontSize: 13, fontWeight: 700 }}
                >
                  {r.description}
                  <span
                    style={{
                ...
                <div style={{ color: '#4a5270', fontSize: 11 }}>
                  {fmt(r.date_added)} {r.notes && `· ${r.notes}`}
                </div>
                {r.receipt_url && (
                  <a
                    href={r.receipt_url}
                    target="_blank"
                    rel="noreferrer"
                    style={{ color: '#4f7cff', fontSize: 11 }}
                  >
                    📷 Fiş
                  </a>
                )}
        ...
        {paidRec.length > 0 && (
          <details style={{ marginTop: 14 }}>
            ...
            >
              ✅ Tahsil Edilenler ({paidRec.length})
            </summary>
            {paidRec.map((r) => (
              <div
                key={r.id}
              ...
              >
                <div style={{ flex: 1 }}>
                  <div style={{ color: '#dde3ef', fontSize: 12 }}>
                    {r.description}
                  </div>
                  <div style={{ color: '#4a5270', fontSize: 10 }}>
                    Tahsil: {fmt(r.paid_date)}
                  </div>
                </div>
                <div
                  style={{ color: '#22c55e', fontWeight: 700, fontSize: 13 }}
                >
                  {r.currency === 'USD' ? '$' : '₺'}
                  {Number(r.amount).toLocaleString()}
                </div>
      ...
      {showAdd && (
        <Modal
          title={editItem ? 'Kaydı Düzenle' : 'Hasta Ekle'}
          onClose={() => {
            setShowAdd(false);
            setEditItem(null);
            resetForm();
        ...
        >
          ...
          >
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <Inp
                ph="Ad Soyad"
                val={form.patient_name}
                set={(v) => setForm((f) => ({ ...f, patient_name: v }))}
              />
            </div>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
              ...
              <Inp
                type="date"
                ph=""
                val={form.surgery_date}
                set={(v) => setForm((f) => ({ ...f, surgery_date: v }))}
              />
            </div>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
            ...
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <Inp
                type="number"
                ph="Örn: 2500"
                val={form.total_price}
                set={(v) => setForm((f) => ({ ...f, total_price: v }))}
              />
            </div>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
              ...
              <Inp
                type="number"
                ph="0"
                val={form.ali_haydar_fee}
                set={(v) => setForm((f) => ({ ...f, ali_haydar_fee: v }))}
              />
            </div>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
      ...
      {showAddRec && (
        <Modal title="Alacak Ekle" onClose={() => setShowAddRec(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
                KİM İÇİN? (kişisel harcama sahibi)
              </div>
              <Sel
                val={recForm.person}
                set={(v) => setRecForm((f) => ({ ...f, person: v }))}
                opts={[
            ...
            <Inp
              ph="Açıklama *"
              val={recForm.description}
              set={(v) => setRecForm((f) => ({ ...f, description: v }))}
            />
            <div
              style={{
                display: 'grid',
            ...
            >
              <Inp
                ...
                type="number"
                val={recForm.amount}
                set={(v) => setRecForm((f) => ({ ...f, amount: v }))}
              />
              <Sel
                val={recForm.currency}
                set={(v) => setRecForm((f) => ({ ...f, currency: v }))}
                opts={[
                  { v: 'TRY', l: '₺ TL' },
                  { v: 'USD', l: '$ USD' },
                  { v: 'SAR', l: 'SAR' },
            ...
            <Inp
              ph="Notlar"
              val={recForm.notes}
              set={(v) => setRecForm((f) => ({ ...f, notes: v }))}
            />
            <div>
              <div style={{ color: '#4a5270', fontSize: 10, marginBottom: 4 }}>
                FİŞ FOTOĞRAFI (opsiyonel)
              </div>
              <input
                type="file"
                accept="image/*"
                onChange={(e) =>
                  setRecForm((f) => ({ ...f, receiptFile: e.target.files[0] }))
                }
                style={{
                  width: '100%',
                  padding: 8,
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn v="s" onClick={() => setShowAddRec(false)}>
                İptal
              </Btn>
              <Btn onClick={saveReceivable} disabled={uploadingReceipt}>
                {uploadingReceipt ? 'Yükleniyor...' : 'Kaydet'}
              </Btn>
            </div>
          </div>
        </Modal>
  ...
  return (
    <div>
      ...
      >
        ...
        <Btn sm onClick={() => setShowAdd(true)}>
          + Gider
        </Btn>
      </div>
      {expenses.map((e) => (
        ...
        >
          ...
          <div style={{ color: '#f04040', fontWeight: 800 }}>
      ...
      {showAdd && (
        <Modal title="Gider Ekle" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <Sel
              val={form.cat}
              set={(v) => setForm((f) => ({ ...f, cat: v }))}
              opts={['Personel', 'Kira', 'Malzeme', 'Reklam', 'Diger']}
            />
            <Inp
              ph="Aciklama *"
              val={form.desc}
              set={(v) => setForm((f) => ({ ...f, desc: v }))}
            />
            <Inp
              ph="Tutar"
              val={form.amount}
              set={(v) => setForm((f) => ({ ...f, amount: v }))}
            />
            <Inp
              type="date"
              ph=""
              val={form.date}
              set={(v) => setForm((f) => ({ ...f, date: v }))}
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn v="s" onClick={() => setShowAdd(false)}>
                Iptal
    ...
    <div>
      ...
      >
        <div style={{ flex: 2, minWidth: 200 }}>
          <Inp ph="Ara..." val={search} set={setSearch} />
        </div>
        <div style={{ flex: 1, minWidth: 160 }}>
          <Sel
            ...
            opts={[
              { v: '', l: '👥 Tümü' },
              ...userNames.map((n) => ({ v: n, l: n })),
            ]}
          />
        </div>
      </div>
      {loading ? (
        <div style={{ color: '#4a5270', textAlign: 'center', padding: 30 }}>
          Yukleniyor...
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ color: '#4a5270', textAlign: 'center', padding: 30 }}>
        ...
        filtered.map((l) => (
          <div
            ...
            style={{
              background: '#121525',
              border: '1px solid #1c2035',
              borderLeft: `3px solid ${actionColor(l.action)}`,
              borderRadius: 10,
              padding: '12px 16px',
              marginBottom: 8,
            }}
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span
                  style={{ color: '#dde3ef', fontWeight: 700, fontSize: 13 }}
                >
                  {l.user_name}
                </span>
                <span
            ...
            <div style={{ color: '#9ba8bc', fontSize: 12, marginLeft: 34 }}>
              {l.target_type}:{' '}
              <span style={{ color: '#dde3ef', fontWeight: 600 }}>
                {l.target_name}
              </span>
              {l.details && ` · ${l.details}`}
            </div>
...
function Settings({ users, setUsers, user, region }) {
  ...
  const changePassword = async () => {
    ...
    setUsers((us) =>
      us.map((u) => (u.id === editUser.id ? { ...u, password: newPass } : u))
    );
    setEditUser(null);
    setNewPass('');
  };
      ...
      {users.map((u) => {
        ...
        return (
          ...
          >
            ...
            {u.role !== 'admin' && (
              <Btn sm v="d" onClick={() => removeUser(u)}>
                Sil
              </Btn>
            )}
          </div>
        ...
        <Modal title="Kullanici Ekle" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <Inp
              ph="Email"
              val={form.email}
              set={(v) => setForm((f) => ({ ...f, email: v }))}
            />
            <Sel
              val={form.role}
              set={(v) => setForm((f) => ({ ...f, role: v }))}
              opts={Object.entries(ROLES).map(([k, v]) => ({
                v: k,
                l: v.badge + ' ' + v.lbl,
              }))}
            />
      ...
      {editUser && (
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn v="s" onClick={() => setEditUser(null)}>
                Iptal
              </Btn>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const connected = await initDrive();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const regionUsers = (all.kullanicilar || []).filter(
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <Logo size={130} />
            <div style={{ color: '#4a5270', fontSize: 12, marginTop: 14 }}>
              Program Şifresi
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {appPassErr && (
              <div
                style={{ color: '#f04040', fontSize: 12, textAlign: 'center' }}
              >
                Sifre yanlis!
              </div>
            )}
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <Logo size={100} />
            <div style={{ color: '#4a5270', fontSize: 13, marginTop: 14 }}>
              🌍 Şube Seçin
            </div>
          </div>
          {Object.entries(REGIONS).map(([key, r]) => (
            <button
              key={key}
              onClick={() => {
                setPendingRegion(key);
                setRegionPass('');
                setRegionPassErr(false);
              }}
              style={{
                padding: '20px',
                borderRadius: 12,
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div
                  style={{ color: '#dde3ef', fontSize: 16, fontWeight: 800 }}
                >
                  {r.lbl}
                </div>
                <div style={{ color: r.clr, fontSize: 11, marginTop: 2 }}>
                  Giriş →
                </div>
              </div>
            </button>
    ...
    return (
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <Logo size={80} />
            <div style={{ fontSize: 48, marginTop: 14, marginBottom: 6 }}>
              {REGIONS[pendingRegion]?.flag}
            </div>
            <div style={{ color: '#c4cdd9', fontSize: 16, fontWeight: 900 }}>
              {REGIONS[pendingRegion]?.lbl}
            </div>
            <div style={{ color: '#4a5270', fontSize: 12, marginTop: 4 }}>
              Şube Şifresi
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Btn
              full
              onClick={() => {
                if (regionPass === REGION_PASSWORDS[pendingRegion]) {
                  setRegion(pendingRegion);
                  setPendingRegion(null);
                  setRegionPass('');
                } else {
                  setRegionPassErr(true);
                }
              }}
            >
              Giris
  ...
  if (!dbReady)
    return (
      ...
      >
        <Logo size={120} />
        <div style={{ color: '#4a5270', fontSize: 12, marginTop: 10 }}>
          {REGIONS[region]?.flag} {REGIONS[region]?.lbl} yükleniyor...
        </div>
      </div>
    );
      ...
      >
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <div
              style={{
                color: REGIONS[region]?.clr,
                fontSize: 13,
                marginTop: 12,
                fontWeight: 700,
              }}
            >
              {REGIONS[region]?.flag} {REGIONS[region]?.lbl}
            </div>
          </div>
          {users.map((u) => {
            const role = ROLES[u.role];
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <div style={{ marginTop: 14 }}>
              <Av
                ...
                size={52}
                clr={ROLES[selectedUser.role]?.clr}
              />
            </div>
            <div
              style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Btn
              full
              onClick={async () => {
                if (userPass === selectedUser.password) {
            ...
            <Btn full v="s" onClick={() => setSelectedUser(null)}>
              Geri
            </Btn>
          </div>
        </div>
  ...
  const visNav = NAV.filter((n) => {
    ...
    if (n.id === 'finance' && region === 'suudi' && user.role !== 'admin')
      return false;
    return true;
  });
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{
            padding: side ? '20px 18px 16px' : '16px 8px',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  gap: 10,
                  width: '100%',
                  padding: side ? '10px 18px' : '10px 0',
                  justifyContent: side ? 'flex-start' : 'center',
                  background: active ? 'rgba(79,124,255,0.12)' : 'transparent',
                  border: 'none',
                  borderLeft: `3px solid ${active ? '#4f7cff' : 'transparent'}`,
                  color: active ? '#4f7cff' : '#4a5270',
                  cursor: 'pointer',
                  fontSize: 13,
        ...
        <div
          style={{
            padding: side ? '14px 18px' : '14px 10px',
        ...
        >
          ...
          {side && (
            <div style={{ flex: 1 }}>
              <div style={{ color: '#dde3ef', fontSize: 12, fontWeight: 600 }}>
                {user.name}
              </div>
              <div style={{ color: '#4a5270', fontSize: 10 }}>
                {ROLES[user.role]?.lbl}
      ...
      <div style={{ flex: 1, padding: '24px 28px', overflowY: 'auto' }}>
        ...
        >
          <button
            ...
            style={{
              padding: '5px 12px',
              background: `${reg.clr}22`,
              border: `1px solid ${reg.clr}`,
              borderRadius: 6,
              color: reg.clr,
              fontSize: 11,
              fontWeight: 700,
          ...
          >
            {reg.flag} {reg.lbl} · Şube Değiştir
          </button>
          {!dbError && (
          ...
          {isAdmin && (
            <>
              <span
                style={{
                  color: driveConnected ? '#22c55e' : '#f97316',
                  fontSize: 11,
                }}
              >
                ● Drive {driveConnected ? 'bagli' : 'bagli degil'}
              </span>
              {!driveConnected && (
                <button
                  onClick={async () => {
        ...
        {page === 'finance' && region === 'suudi' && user.role === 'admin' && (
          <SuudiFinance
            user={user}
            region={region}
            patients={patients}
